...
function App() {
	...
	useEffect(() => {
		...
		socket.on(SOCKET_EVENTS.GAME_STATE, (game) => {
			...
			console.log('🎮 Current player data:', game?.players?.find(p => p.id === currentPlayerId));
			setGame(game);

			// ✅ Update current player data if found
			const currentPlayerData = game?.players?.find(p => p.id === currentPlayerId);
		...
		socket.on(SOCKET_EVENTS.PLAYER_JOINED, (data: { playerId: string; playerData: any }) => {
			console.log('👥 Player joined:', data);
			addNotification('success', 'Гравець приєднався', `${data.playerData?.name || 'Гравець'} приєднався до гри.`);
		});
		...
		socket.on('player-ready-status-changed' as any, (data: { playerId: string; isReady: boolean }) => {
			console.log('🔄 Player ready status changed:', data);
			addNotification('info', 'Статус змінено', `Гравець ${data.isReady ? 'готовий' : 'не готовий'} до гри.`);
		});
	...
	const addNotification = (type: 'info' | 'success' | 'warning' | 'error', title: string, message: string, duration?: number) => {
		switch (type) {
			case 'success':
				success(title, message, duration); break;
			case 'error':
				error(title, message, duration); break;
			case 'warning':
				warning(title, message, duration); break;
			case 'info':
			default:
				info(title, message, duration); break;
		}
	};
	...
	const createGame = async () => {
		...
		if (!playerName.trim()) {
			...
			addNotification('warning', 'Введіть ім\'я', 'Будь ласка, введіть ваше ім\'я');
			return;
		}

		if (!currentPlayerId) {
			console.log('❌ Player ID is missing');
			addNotification('error', 'Помилка ідентифікації', 'Відсутній ідентифікатор гравця');
			return;
		}
		...
		try {
			...
			if (data.success) {
				...
				addNotification('success', 'Гру створено', `Гра створена успішно! ID: ${createdGameId}`);
			} else {
				console.log('❌ Game creation failed:', data);
				addNotification('error', 'Помилка створення гри', data.error || 'Невідома помилка');
			}
		} catch (error) {
			console.error('💥 Error creating game:', error);
			addNotification('error', 'Помилка мережі', 'Не вдалося створити гру. Спробуйте ще раз.');
		} finally {
	...
	const joinExistingGame = async (gameIdToJoin: string) => {
		try {
			...
			const response = await fetch(`http://localhost:3001/api/games/${gameIdToJoin}/join`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
				},
			...
			if (data.success) {
				console.log('✅ Successfully joined via API, now connecting Socket.IO...');
				...
				addNotification('error', 'Помилка приєднання', data.error || 'Не вдалося приєднатися до гри');
				return false;
			}
		} catch (error) {
			console.error('Error joining game:', error);
			addNotification('error', 'Помилка мережі', 'Не вдалося з\'єднатися з сервером');
			return false;
		}
	};
	...
	const joinGame = async () => {
		if (!joinGameId.trim()) {
			addNotification('warning', 'ID гри не введено', 'Введіть ID гри для приєднання');
			return;
		}
		...
		if (success) {
			addNotification('success', 'Приєднання успішне', `Ви приєдналися до гри ${joinGameId.trim()}`);
		}
	};
	...
	const joinGameSocket = (gameIdToJoin: string) => {
		...
		console.log('🔌 Player details:', { playerId: currentPlayerId, playerName: playerName.trim() });
		...
		if (socketService.isGameConnected) {
			...
			socketService.joinGame(gameIdToJoin, currentPlayerId, playerName.trim() || 'Player');
			setGameId(gameIdToJoin);
			...
			setTimeout(() => {
				...
				if (socketService.isGameConnected) {
					...
					addNotification('error', 'Не підключено до сервера', 'Не вдалося приєднатися до гри, спробуйте ще раз.');
				}
			}, 1000);
		}
	};
	...
	const startSinglePlayerMode = () => {
		...
		addNotification('success', 'Гра розпочата!', 'Ви граєте у режимі одного гравця');
	...
	const startDeveloperMode = () => {
		...
		addNotification('info', 'Тестовий режим', 'Запущено тестовий режим з панеллю для розробників');
						...
						<div className="card board-card">
							...
							<div className="connection-status">
								...
								<div className="debug-info">
									<small>Player ID: {currentPlayerId?.substring(0, 12)}...</small>
									<small>Socket: {socketService.isGameConnected ? '✅' : '❌'}</small>
						...
						<div className="lobby-content">
										...
										<div className="action-card create-card">
											<div className="card-body">
												<div className="feature-list">
													<span className="feature">⚡ Миттєве створення</span>
												...
												<button
													onClick={() => {
														...
														const isDisabled = connectionStatus !== 'Підключено' || !playerName.trim() || isCreatingGame;
														console.log('- button disabled:', isDisabled);
														
														if (!isDisabled) {
															createGame();
														} else {
															console.log('❌ Button is disabled, cannot create game');
														}
													}}
													disabled={connectionStatus !== 'Підключено' || !playerName.trim() || isCreatingGame}
												...
												{connectionStatus !== 'Підключено' && (
													<button
														onClick={async () => {
															...
															console.log('🧪 TESTING: Forcing game creation without Socket.IO');
															await createGame();
														}}
														disabled={!playerName.trim() || isCreatingGame}
														className="section-btn create-btn"
														style={{ background: '#ff6b6b', marginTop: '8px' }}
									...
									<div className="section-content">
										<div className="action-card join-card">
											<div className="card-body">
												...
												<button
													onClick={joinGame}
													disabled={connectionStatus !== 'Підключено' || !playerName.trim() || !joinGameId.trim() || isJoiningGame}
							...
							<div className="info-section">
								<div className="info-card">
									<h4>Про гру</h4>
									<p>Cash Flow Ukraine - це онлайн версія популярної настільної гри, де гравці вчаться управляти фінансами та інвестувати кошти.</p>
								</div>
								...
								<div className="info-card rules-card">
									...
									<div className="rules-content">
										<div className="rules-section">
											<h5>🎯 Мета гри:</h5>
											<p>Вийти з "пастки для щурів" та досягти фінансової свободи через пасивний дохід</p>
										</div>
										...
										<div className="rules-section">
											<h5>💰 Фінанси:</h5>
											<ul>
												<li>Пасивний дохід {'>'}  Витрати = Фінансова свобода</li>
										...
										<div className="rules-section">
											<h5>🏆 Перемога:</h5>
											<p>Перший гравець, який досягне фінансової свободи, перемагає!</p>
										</div>
									</div>
								</div>
								...
								<div className="info-card">
									<h4>Як грати</h4>
									<ul>
										<li><strong>🎮 Грати в одного:</strong> Фінальна версія гри без панелей розробника - як у реальному випуску</li>
										<li><strong>👥 Мультиплеєр:</strong> Створіть гру або приєднайтеся за ID для гри з друзями</li>
										<li><strong>🧪 Тестування:</strong> Режим розробника з додатковими панелями для налагодження</li>
				...
				<div className="game-container-">
					...
					{storeGameData ? (
						<main className="game-content">
							...
							{(storeGameData.state === 'waiting' || storeGameData.state === 'starting') ? (
								<GameLobby
									game={storeGameData}
									gameId={gameId}
									playerId={currentPlayerId} // ✅ Fixed: Use consistent playerId
									onNotification={(notification) => {
										addNotification(notification.type, notification.title, notification.message, notification.duration);
									}}
