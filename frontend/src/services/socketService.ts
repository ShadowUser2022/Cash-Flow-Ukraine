...
class SocketService {
	private gameSocket: Socket<ServerToClientEvents, ClientToServerEvents> | null = null;
	private webrtcSocket: Socket<ServerToClientEvents, ClientToServerEvents> | null = null;
	...
	charityChoice(gameId: string, playerId: string, choice: string, amount?: number) {
		if (!this.gameSocket) {
			throw new Error('Game socket not connected');
		}
	...
	receiveIncome(gameId: string, playerId: string, amount: number, description?: string) {
		if (!this.gameSocket) {
			throw new Error('Game socket not connected');
		}
	...
	getPlayerFinances(gameId: string, playerId: string): Promise<any> {
		...
		return new Promise((resolve, reject) => {
			this.gameSocket!.emit(SOCKET_EVENTS.GET_PLAYER_FINANCES as any, {
				gameId,
				playerId
			}, (response: any) => {
				if (response.success) {
	...
	getPlayerDream(gameId: string, playerId: string): Promise<{ dream: any; hasSelected: boolean }> {
		if (!this.gameSocket) {
			throw new Error('Game socket not connected');
		}

		return new Promise((resolve, reject) => {
			this.gameSocket!.emit(SOCKET_EVENTS.GET_PLAYER_DREAM as any, {
				gameId,
				playerId
			}, (response: any) => {
				if (response.success) {
