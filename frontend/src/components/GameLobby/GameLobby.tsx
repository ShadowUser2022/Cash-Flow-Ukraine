...
const GameLobby: React.FC<GameLobbyProps> = ({
	...
	const [selectedDream, setSelectedDream] = useState<Dream | undefined>(undefined);
	const [showDreamModal, setShowDreamModal] = useState(false);

	const currentPlayer = game?.players.find(p => p.id === playerId);
	const isHost = game?.hostId === playerId;
	const allPlayersReady = game?.players.length >= 1 && game?.players.every(p => p.isReady);
	...
	useEffect(() => {
		...
		socket.on(SOCKET_EVENTS.PLAYER_READY, (data: { playerId: string; isReady: boolean }) => {
			const playerName = game.players.find(p => p.id === data.playerId)?.name;
			onNotification({
				type: 'info',
		...
		socket.on(SOCKET_EVENTS.PROFESSION_SELECTED, (data: { playerId: string; profession: string; playerName: string }) => {
		...
		socket.on(SOCKET_EVENTS.PLAYER_REMOVED, (data: { playerId: string; playerName: string }) => {
			onNotification({
				type: 'warning',
	...
	const handleProfessionChange = async (professionName: string) => {
		...
		try {
			const response = await fetch(`http://localhost:3001/api/games/${gameId}/profession`, {
				method: 'POST',
				headers: {
	...
	const handleReadyToggle = async () => {
		...
		try {
			...
			const response = await fetch(`http://localhost:3001/api/games/${gameId}/ready`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
				},
			...
			if (response.ok) {
				...
				onNotification({
					...
					message: newReadyState ? 'Ви готові розпочати гру' : 'Ви скасували готовність',
					duration: 3000
				});
			} else {
				const error = await response.json();
	...
	const handleRemovePlayer = async (playerIdToRemove: string) => {
		...
		try {
			const response = await fetch(`http://localhost:3001/api/games/${gameId}/remove-player`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
				},
		...
		<div className="game-lobby">
				...
				<div className="players-section">
					...
					<div className="players-list">
						{game.players.map((player) => {
							...
							return (
								<div key={player.id} className={`card lobby-card ${isCurrentPlayer ? 'glow' : ''} ${player.isReady ? 'success-card' : 'warning-card'}`}>
									<div className="player-info">
										<div className="player-name">
											{player.name}
											{isCurrentPlayer && <span className="you-label">(Ви)</span>}
											{game.hostId === player.id && <span className="host-label">👑 Хост</span>}
										</div>
										...
										<div className="player-profession">
											...
											{profession && (
												<div className="profession-details">
													<span>Зарплата: {formatCurrency(profession.salary)}</span>
													<span>Витрати: {formatCurrency(profession.expenses)}</span>
												</div>
											)}
										</div>
					...
					<div className="card action-card">
						...
						<div className="dream-control">
							...
							<button
								...
								disabled={isReady}
							>
								{selectedDream ? `${selectedDream.icon} ${selectedDream.title}` : '🌟 Обрати мрію'}
							</button>
							{selectedDream && (
								<div className="dream-details">
									<span className="dream-goal">Ціль: ${selectedDream.estimatedCost.toLocaleString()}</span>
								</div>
							)}
							{isReady && (
								<span className="disabled-hint">
			...
			{showProfessionModal && (
				<div className="modal-overlay" onClick={() => setShowProfessionModal(false)}>
					<div className="card modal-card profession-modal" onClick={(e) => e.stopPropagation()}>
								...
								>
									<div className="profession-info">
										...
										<div className="profession-stats">
											<span>💰 Зарплата: {formatCurrency(profession.salary)}</span>
											<span>💸 Витрати: {formatCurrency(profession.expenses)}</span>
											<span>💵 Залишок: {formatCurrency(profession.salary - profession.expenses)}</span>
										</div>
									</div>
								</div>
							))}
