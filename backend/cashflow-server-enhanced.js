...
app.post('/api/games/:gameId/join', (req, res) => {
	...
	if (existingPlayerIndex >= 0) {
		...
		game.players[existingPlayerIndex] = { ...game.players[existingPlayerIndex], ...playerData };
	} else {
		// Add new player
	...
	console.log('🎮 Updated game state:', { id: game.id, playersCount: game.players.length, state: game.state });

	// ✅ Emit game state to all connected clients in this game room
	if (io) {
		io.to(`game-${gameId}`).emit('game-state', game);
		io.to(`game-${gameId}`).emit('player-joined', { playerId, playerData });
	}
...
app.post('/api/games/:gameId/ready', (req, res) => {
	...
	if (io) {
		io.to(`game-${gameId}`).emit('game-state', game);
		io.to(`game-${gameId}`).emit('player-ready-status-changed', { playerId, isReady });
	}
...
app.post('/api/games/:gameId/profession', (req, res) => {
	...
	const professions = {
		'Вчитель': { name: 'Вчитель', salary: 3300, expenses: 2500, description: 'Освітній працівник' },
		'Модератка': { name: 'Модератка', salary: 4100, expenses: 2800, description: 'Контент-модератор' },
		'Поліцейський': { name: 'Поліцейський', salary: 3000, expenses: 2200, description: 'Правоохоронець' },
		'Інженер': { name: 'Інженер', salary: 4900, expenses: 3900, description: 'Технічний спеціаліст' },
		'Пілот': { name: 'Пілот', salary: 13200, expenses: 9650, description: 'Авіапілот' },
		'Юрист': { name: 'Юрист', salary: 7500, expenses: 6100, description: 'Правовий консультант' }
	};
	...
	if (io) {
		io.to(`game-${gameId}`).emit('game-state', game);
		io.to(`game-${gameId}`).emit('profession-changed', { playerId, profession: professionName });
	}
...
function updatePlayerCash(playerId, amount, type, description) {
	...
	console.log(`💰 Player ${playerId}: ${description} | ${amount >= 0 ? '+' : ''}$${amount} | Balance: $${oldCash} → $${finances.cash}`);
...
gameNamespace.on('connection', (socket) => {
	...
	socket.on('join-game', ({ gameId, playerId, playerName }) => {
		...
		if (!gameData) {
			console.log(`⚠️ Game ${gameId} not found in memory, creating basic game state...`);
	...
	socket.on('ready-to-start', ({ gameId, playerId, isReady }) => {
		...
		socket.to(gameId).emit('player-ready-status-changed', { playerId, isReady });
	});
	...
	socket.on('execute-turn', ({ gameId, playerId }) => {
		console.log(`🎯 Executing full turn for player ${playerId} in game ${gameId}`);
		...
		try {
			...
			if (Math.random() > 0.7) {
				const effects = [
					{
						type: 'pay_money',
						data: { amount: 500, description: 'Штраф за перевищення швидкості' }
					},
					{
						type: 'receive_money',
			...
			console.log(`🎲 Dice result: ${diceResult}, Position: ${currentPosition} → ${newPosition}, Cash: $${currentFinances.cash}`);
			...
			socket.emit('error', { message: 'Failed to execute turn', error: error.message });
		}
	});

	socket.on('turn-ended', ({ gameId, playerId, nextPlayer }) => {
		console.log(`Turn ended for ${playerId}, next: ${nextPlayer} in game ${gameId}`);
	...
	socket.on('pay-expense', ({ gameId, playerId, amount }) => {
		...
		try {
			...
			const result = updatePlayerCash(playerId, -amount, 'expense', 'Незаплановані витрати');
	...
	socket.on('receive-income', ({ gameId, playerId, amount, description = 'Дохід' }) => {
		console.log(`💰 Player ${playerId} receiving income $${amount} in game ${gameId}`);

		try {
			const result = updatePlayerCash(playerId, amount, 'income', description);
	...
	socket.on('charity-choice', ({ gameId, playerId, choice, amount }) => {
		console.log(`❤️ Player ${playerId} charity choice: ${choice}, amount: $${amount} in game ${gameId}`);
		...
		try {
			...
			if (choice !== 'skip' && amount > 0) {
				...
				result = updatePlayerCash(playerId, -amount, 'charity', 'Благодійний внесок');
	...
	socket.on('market-action', ({ gameId, playerId, action, data }) => {
		console.log(`📈 Player ${playerId} market action: ${action} in game ${gameId}`);
		...
		try {
			...
			if (action === 'execute' && data?.card) {
				...
				if (card.benefit && card.benefit.amount) {
					...
					result = updatePlayerCash(playerId, card.benefit.amount, 'market', card.title || 'Ринкова подія');
					...
					result = updatePlayerCash(playerId, -card.cost, 'market', card.title || 'Ринкова подія');
	...
	socket.on('collect-salary', ({ gameId, playerId }) => {
		...
		try {
			...
			const result = updatePlayerCash(playerId, salaryAmount, 'salary', 'Місячна зарплата');
	...
	socket.on('set-player-dream', ({ gameId, playerId, dream }) => {
		console.log(`💭 Player ${playerId} setting dream in game ${gameId}:`, dream);
	...
	socket.on('get-player-finances', ({ gameId, playerId }, callback) => {
		console.log(`📊 Requesting finances for player ${playerId} in game ${gameId}`);
	...
	socket.on('disconnect', () => {
		...
		if (user) {
			...
			socket.to(user.gameId).emit('player-disconnected', { playerId: user.playerId });
			connectedUsers.delete(socket.id);
		}
